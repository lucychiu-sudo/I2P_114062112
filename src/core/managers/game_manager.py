...
class GameManager:
    ...
    def __init__(self, maps: dict[str, Map], start_map: str, 
        ...
        self.next_map = ""
        self._return_positions: dict[str, Position] = {}
    ...
    def current_shop_npc(self) -> list[ShopNPC]:
        return self.shop_npc[self.current_map_key]
    ...
    def switch_map(self, target: str) -> None:
        if target not in self.maps:
            ...
            return
        if self.player:
            self._return_positions[self.current_map_key] = self.player.position.copy()
    ...
    def try_switch_map(self) -> None:
        ...
        if self.should_change_scene:
            ...
            self.should_change_scene = False
            if self.player:
                if self.current_map_key in self._return_positions:
                    self.player.position = self._return_positions[self.current_map_key]
                else:
    ...
    def check_collision(self, rect: pg.Rect) -> bool:
        ...
        for entity in self.enemy_trainers[self.current_map_key]:
            if rect.colliderect(entity.animation.rect):
                return True
        for entity in self.shop_npc[self.current_map_key]:
            if rect.colliderect(entity.animation.rect):
                return True
        return False
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        for key, m in self.maps.items():
            ...
            block["shop_npc"] = [t.to_dict() for t in self.shop_npc.get(key, [])]
    ...
    def from_dict(cls, data: dict[str, object]) -> "GameManager":
        ...
        Logger.info("Loading shop npc")
        for m in data["map"]:
            raw_data = m["shop_npc"]
            gm.shop_npc[m["path"]] = [ShopNPC.from_dict(t, gm) for t in raw_data]
